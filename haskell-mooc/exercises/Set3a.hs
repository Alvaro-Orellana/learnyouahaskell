...
maxBy :: (a -> Int) -> a -> a -> a
maxBy measure a b = if aIsBigger then a else b
                    where aIsBigger = measure a > measure b
...
firstHalf xs = take limit xs 
        where limit = if odd (length xs) 
...
powers :: Int -> Int -> [Int]
powers k max = let powersOfK = (map (k^) [0..])
               in  takeWhile (<= max) powersOfK
...
joinToLength n xs = [newArray  | string <- xs, newArray <- map (string ++) xs,length newArray == n]
