...
countHelper value counter = maybe (counter+1) (\x -> counter) value
...
slHelper counter (sum, length) = (sum+counter, length+1)
...
largestHelper value [] = [value]
largestHelper value xs 
    | value > maximum xs  = [value]
    | maximum xs == value = value:xs
    | otherwise           = xs
