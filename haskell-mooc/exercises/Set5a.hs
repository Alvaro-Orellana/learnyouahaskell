...
study student = case student of 
    Freshman     -> NthYear 1
    NthYear year -> if year <= 6 then NthYear (year+1) else Graduated
    Graduated    -> Graduated
...
tick :: UpDown -> UpDown
tick ud = case ud of 
    Increasing counter -> Increasing (counter+1)
    Decreasing counter -> Decreasing (counter-1)

-- toggle changes an increasing counter into a decreasing counter and
-- vice versa
toggle :: UpDown -> UpDown
toggle ud = case ud of 
    Increasing counter -> Decreasing counter
    Decreasing counter -> Increasing counter
...
rgb color = case color of
    ...
    Green -> [0,1,0]
    Blue  -> [0,0,1]
    Mix color1 color2 -> average (rgb color1) (rgb color2)
    Invert col -> invert (rgb col)
    where average color1 color2 = map (\(a,b) -> (a+b) / 2)  (zip color1 color2)
          invert col = map (\x -> abs (1-x)) col
...
fromNat :: Nat -> Int
fromNat Zero = 0
fromNat (PlusOne number) = 1 + fromNat number
...
fromBin :: Bin -> Int
fromBin bin = fromBin' 0 bin
  where fromBin' n (O b) = fromBin' (n+1) b
        fromBin' n (I b) = 2^n + fromBin' (n+1) b
        fromBin' n End   = 0

toBin :: Int -> Bin
toBin 0 = O End
toBin i = toBin' End 0 i
  where toBin' bin n i 
          | n == i = bin
          | otherwise =  toBin' (inc bin) (n+1) i